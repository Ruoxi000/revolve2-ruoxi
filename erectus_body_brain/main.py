...
def run_experiment(dbengine: Engine) -> None:
    ...
    if getattr(config, "SEED_DATABASE_FILE", None):
        try:
            seed_path = Path(config.SEED_DATABASE_FILE).expanduser().resolve()
            if not seed_path.exists():
                raise FileNotFoundError(f"path not found: {seed_path}")

            logging.info(f"Loading top {config.SEED_TOP_K} unique genotypes from {seed_path}.")
            seed_engine = open_database_sqlite(str(seed_path), open_method=OpenMethod.OPEN_IF_EXISTS)

            with Session(seed_engine) as seed_session:
                # UNIQUE by genotype, pick each genotype's *best ever* fitness
                stmt = (
                    select(Genotype, func.max(Individual.fitness).label("best_fit"))
                    .join(Individual, Individual.genotype_id == Genotype.id)
                    .group_by(Genotype.id)
                    .order_by(desc("best_fit"))
                    .limit(int(config.SEED_TOP_K))
                )
                rows = seed_session.execute(stmt).all()

            seed_genotypes = [g for (g, _best) in rows]
            seed_engine.dispose()

        except Exception as e:
            logging.warning(f"Failed to load seed population from {config.SEED_DATABASE_FILE}: {e}")
