...
w_move_max_values = [4.0]
height_groups = [
    #("low", 0.03, 0.03, 0.20),
    ("high", 0.06, 0.06, 0.25),
]
# Include hybrid optimizer option in addition to pure DE and CMA-ES
# optimizers = ["de", "cmaes", "hybrid"]
optimizers = ["de"]

def run_all() -> None:
    """Run all combinations of bodies, speed weights, heights and optimizers."""

    # keep runs short when used as a quick comparison
    config.HYBRID_DE_GENERATIONS = 300
    config.HYBRID_CMA_GENERATIONS = 0
    config.NUM_GENERATIONS = config.HYBRID_DE_GENERATIONS + config.HYBRID_CMA_GENERATIONS
